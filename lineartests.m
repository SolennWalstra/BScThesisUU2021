...
subplot(1, 3, 2)
plot(range,i_amplitude,'.','Color', [1, .65, .0], 'MarkerSize', 15)
hold on
...
xlabel('Influencer')
ylabel('Amplitude (°)')
title('Amplitudes per Influencer')
axis('square')
set(gca, 'FontName', 'Times New Roman');
...
distance=[1;2;3];
table_distance=table(d_amplitude,distance);
%%% linear regression for target amplitude + predicted vals
...
subplot(1,3,3)
set(gca, 'FontName', 'Times New Roman');
plot(range_distance,d_amplitude,'.', 'Color',[0.8 0.2 0.6 .6], 'MarkerSize', 15)
hold on
